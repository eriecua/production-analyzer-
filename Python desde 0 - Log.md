...
## [2026-08-17 12:00] bloque-validado | S12-B4

- Evidencia: `analizador-produccion/analizador_produccion.py`, sin cambios en este bloque. El entregable es la demostración y la retrospectiva.
- Verificación: **cinco ejecuciones observadas en esta sesión, todas con `; $LASTEXITCODE` a la vista.** `demo.csv` da 5 válidos, 1 rechazado y código 0, con los nueve números de `salidas/resumen_por_turno.csv` coincidiendo con `datos/demo_esperado.md` sobre un reporte recién generado. `produccion.csv` da 5/1 y código 0, con `git status --short` sin salida: el original intacto. `prueba_falta_columna.csv` da código 4 con mensaje limpio; `prueba_campo_vacio.csv` da 1 válido, 1 rechazado y código 0; `produccion_falla.csv` da código 4. Ningún traceback. `python -m pytest tests/` da **11 passed**.
- Nota: [[45 - S12-B4 - Demostracion y retrospectiva]]
- Índice actualizado: sí.
- **Estado: validado. La Semana 12 NO queda completa:** la `VALIDACIÓN SEMANAL 12` sigue abierta y pide que *otra persona* ejecute el programa usando solo el README. Al cierre de este bloque no tiene a nadie todavía.
- **Hallazgo del bloque, y es su propio diagnóstico: los conceptos por separado están, lo que cuesta es la integración de uno dentro de otro.** Sus palabras: «los conceptos uno por uno los sé, pero la integración de esos conceptos, un concepto dentro de otro concepto, es lo que más me dificulta». Nombró tres dificultades por separado —diccionarios, acumuladores, `for`— y vio solo que las tres son la misma. Explica por qué `datos[turno]["unidades"]` de S11-B4 costó y por qué la segunda función salió sin esqueleto. **Debe gobernar el diseño de la ruta de pandas: ejercicios de composición desde el principio, no conceptos aislados.**
- **Otra conclusión suya sobre el proyecto completo: «el 20 % es código y el 80 % es manejo de errores.»** Nadie la formula en la Semana 1.
- **Deuda de retención confirmada por él, segunda vez que aparece.** Sobre `try`: sabe para qué sirve y no recuerda cómo se escribe, aunque escribió cuatro `except` en este programa y el último por transferencia. Es procedimental, no conceptual: se recupera reconstruyendo de memoria, no releyendo. Ya se vio el 2026-08-13 con la validación de campo vacío. Conviene repasar su propio código al abrir material nuevo.
- **Error del tutor, corregido mirando el archivo.** Ante `Falta la columna obligatoria: id_registro` de `produccion_falla.csv` se sospechó el BOM de Excel, porque el síntoma es idéntico. `Get-Content -TotalCount 1` mostró que la cabecera real es `turno,producto,horas_trabajadas`: le faltan cinco columnas de verdad y el programa se comportaba bien. Lo erróneo era suponer que ese archivo probaba filas malas.
- **Se le propuso hacer él mismo la validación semanal y se rechazó, con motivo.** Ejecutar su propio programa «usando solo el README» no mide nada: ya sabe lo que el README no dice. Es el problema del chat contaminado por cuarta vez. Se le ofrecieron dos vías: una persona con Python instalado, o un chat de Claude en frío con solo el README.
- **Corrección de registro.** Las entradas de hoy a las 10:00 y 11:00 dicen que S12-B1 y S12-B3 estaban «cumplidos de facto». El plan de ProjectLibre, visto en pantalla en esta sesión, marca **S12-B3 al 0 %** («Escribir instrucciones en README.md»). El README existe y él lo revisó línea por línea el 2026-08-13, pero el porcentaje lo lleva él. No se modifican aquellas entradas: queda corregido desde esta.
- Observación sin tarea: el programa reporta **una** columna faltante y no la lista completa. El mensaje es cierto y el código correcto; el proyecto está cerrado y no se reabre.
